...
target_path = f"s3://{bucket}/transform_data/weather/future/"

# 공항 코드 목록
airports = ["ICN", "NRT", "FUK", "KIX", "CTS"]

dataframes = []
# Incremental Update: Check if target folder exists
try:
...
while current_date <= execution_date:
    ...
    for airport in airports:
        try:
            ...
            if latest_timestamp:
                files = files.filter(to_timestamp(col("timestamp")) > latest_timestamp)
            if not files.rdd.isEmpty():
                dataframes.append(files)
            else:
                print(f"No valid data for {airport} in {raw_path}. Skipping.")
        except Exception as e:
...
if combined_df:
    ...
    if not cleaned_df.rdd.isEmpty():
        cleaned_df.coalesce(1).write.mode("append").parquet(target_path)
